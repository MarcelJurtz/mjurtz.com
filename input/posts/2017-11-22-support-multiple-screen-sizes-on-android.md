...
```javascreenLayout = getResources().getConfiguration().screenLayout & Configuration.SCREENLAYOUT_SIZE_MASK;```

This returns an integer value. By using the following constants, I make it easier for myself to differentiate between them:

```javaprivate final int SCREEN_SMALL = 1;
...
```javarepositionControlsOnToggle = screenLayout != SCREEN_XLARGE && view.GetPlayerAmount() == 4;```
